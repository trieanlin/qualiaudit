...
describe('printable HTML audit report', () => {
  ...
  it('builds a self-contained, privacy-minimised report by default', () => {
    ...
    expect(report).toContain('QualiAudit HTML v0.4')
    expect(report).toContain('attention organisation only')
    expect(report).toContain('no batch resolution or automatic recoding')
