...
| LUC-SAF-03-R1 | Retest both case views after allowing unbroken code identifiers to wrap inside bounded comparison columns. No first- or second-human code may overlap the other column at desktop or narrow widths. | Pass | Retested in the Vercel branch preview with Safari on 2026-07-30. The long English first-human code wrapped within its column without covering the Chinese second-human code. After narrowing the Safari window, the identifiers continued to wrap and the comparison reflowed without overlap or lost content. |
| LUC-SAF-04 | Resolve one case with a deliberately long mixed-language rationale, then inspect the decision log and codebook-change or memo controls that become available. Saved content remains readable and associated with the correct case. | Pass | Tested in the Vercel branch preview with Safari on 2026-07-30. A `Revise codebook` decision was saved for `LONG-UNICODE-001` with a long mixed Chinese-English rationale. The decision log preserved the complete rationale, long human and final code identifiers, AI code, timestamp, and changed-after-exposure status without overlap or truncation. The corresponding codebook revision remained attached to the same case and readable in the audit trail. |
| LUC-SAF-05 | Export reviewed CSV, JSON audit, and an HTML report containing source text. Reopen or inspect each file and confirm the complete long identifiers and Chinese text remain present and readable without character corruption. | Pass | Tested from the Vercel branch preview with Safari on 2026-07-30. The reviewed CSV, JSON audit, and HTML report with full source text were downloaded and reopened. The long identifiers, Chinese source material, mixed-language decision rationale, and associated audit content remained complete and readable without character corruption, clipping, or overlap. |
