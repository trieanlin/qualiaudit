...
Do not use the current prototype with sensitive, identifiable, embargoed, or regulated research material. The optional real-model adapter now implements an initial server-side secret boundary, exact-field allowlist, provider disclosure, explicit consent, output validation, and local fallback. It has not completed a formal threat model or institutional governance review and is not described as suitable for real research data. See [Optional real reviewer](docs/REMOTE_REVIEWER.md).
